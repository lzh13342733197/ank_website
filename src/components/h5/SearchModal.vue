<template>
  <Transition name="search-modal">
    <div class="search-modal" v-show="isVisible" @click.self="closeSearch">
        ...
        <div class="search-results" v-if="searchResults.length > 0">
          ...
          <div class="results-list">
            <div v-for="(result, index) in searchResults" :key="result.id" class="result-item"
              :class="{ active: activeIndex === index }" @click="selectResult(result)"
...
const selectResult = (result: SearchResult) => {
  ...
  if (result && result.productSpuId)
    router.push(`/mobile/product-detailInfo?id=${result.productSpuId}`)
  closeSearch()
}
