...
const getblogDataList = async () => {
  try {
    isLoading.value = true
    const data = await useFetchWithLanguage.post(`${import.meta.env.VITE_API_URL}/article/pageList`, {
      page: 1,
      limit: 10
    })
    blogPostsList.value = data?.list || []
  } catch (error) {
    console.error(error)
  } finally {
...
.modal-mask {
  ...
  align-items: flex-start; z-index: 9999; overflow-y: auto;
  padding: 40px 20px; /* PC端留白 */
}

.modal-content {
  background: #fff; border-radius: 16px; max-width: 800px; width: 100%;
  position: relative; padding: 40px; box-shadow: 0 20px 40px rgba(0,0,0,0.3);
}
...
.modal-title { font-size: 1.8rem; margin-bottom: 20px; line-height: 1.3; color: #111; padding-right: 40px; }

/* --- 富文本换行修复 --- */
.modal-content-html {
  line-height: 1.7; color: #333; font-size: 16px;
...
.modal-content-html :deep(p) { margin-bottom: 1.2em; }
.modal-content-html :deep(img), .modal-content-html :deep(video) {
  max-width: 100% !important; height: auto !important; border-radius: 8px; margin: 10px 0;
}
...
@media (max-width: 768px) {
  ...
  .blog-list { grid-template-columns: 1fr; } /* 移动端一行一个 */
  ...
  .modal-content {
    min-height: 100%; /* 撑满屏幕高度 */
    border-radius: 0; /* 移动端全屏不需要圆角 */
    padding: 60px 20px 30px; /* 顶部留出位置给关闭按钮 */
  }

  .modal-close {
    position: fixed; /* 滚动时关闭按钮固定在顶部 */
    top: 15px;
    right: 15px;
    width: 40px; /* 稍微加大方便点击 */
    height: 40px;
